...
public class StockTableModel extends TableFieldInfoModel {
    ...
    public void moveTop(int modelRowIndex) {
        if (modelRowIndex < 0) {
            return;
        }
    ...
    public void moveBottom(int modelRowIndex) {
        if (modelRowIndex < 0) {
            return;
        }
    ...
    public void moveUp(int modelRowIndex) {
        if (modelRowIndex < 0) {
            return;
        }
    ...
    public void moveDown(int modelRowIndex) {
        StockDataBean stockDataBean = getStockDataBean(modelRowIndex);
        List<StockDataBean> list = stockDataBeanMap.values().stream()
                .filter(s -> s.isPinTop() == stockDataBean.isPinTop())
                .sorted(Comparator.comparing(StockDataBean::getIndex))
